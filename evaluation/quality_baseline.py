"""Single-graph quality baseline harness (self-documenting edition).

Records the independent variables a result must be attributable to: provider, model,
family, temperature, thinking_mode, git commit AND whether the working tree was dirty,
plus the actual error messages behind any n_errors count. Aggregates grounding metrics
over repeats with mean +/- stdev (temperature makes memos stochastic; a delta inside the
stdev is noise, not signal).
"""
...
async def evaluate_single_run(agent, ticker, *, include_filing_analysis, include_news_sentiment, max_news_articles) -> dict:
    ...
    # Preserve error provenance so a deterministic n_errors is diagnosable from the artifact.
    error_detail = [{"step": e.get("step"), "message": (e.get("message") or "")[:200]} for e in errors]

    return {
        "ticker": ticker,
        "passed": bool(vr.get("passed", False)),
        "total_claims": int(vr.get("total_claims", 0)),
        "cited_claims": int(vr.get("cited_claims", 0)),
        "grounded_claims": int(vr.get("grounded_claims", 0)),
        "citation_coverage_rate": float(vr.get("citation_coverage_rate", 0.0)),
        "grounded_claim_rate": float(vr.get("grounded_claim_rate", 0.0)),
        "orphan_citations": len(vr.get("orphan_citations", []) or []),
...
def _aggregate(runs: list[dict]) -> dict:
    ...
    return {
        ...
        "grounded_claim_rate": _stats(col("grounded_claim_rate")),
        "citation_coverage_rate": _stats(col("citation_coverage_rate")),
        "total_claims": _stats(col("total_claims")),
        "cited_claims": _stats(col("cited_claims")),
        "grounded_claims": _stats(col("grounded_claims")),
        "orphan_citations": _stats(col("orphan_citations")),
        "evidence_count": _stats(col("evidence_count")),
        "filing_chunks": _stats(col("filing_chunks")),
        "news_articles": _stats(col("news_articles")),
        "memo_words": _stats(col("memo_words")),
        "run_ms": _stats(col("run_ms")),
    }
...
async def main() -> None:
    ...
    for ticker in args.tickers:
        for i in range(args.repeats):
            ...
            print(f"  {ticker} run {i}: grounded={r['grounded_claim_rate']:.2f} "
                  f"coverage={r['citation_coverage_rate']:.2f} claims={r['total_claims']} "
                  f"errors={r['n_errors']} passed={r['passed']}")
    ...
    dirty = " (WORKING TREE DIRTY - uncommitted changes)" if artifact["git"]["dirty"] else ""
    print("\n=== QUALITY BASELINE ===")
    ...
    print(f"error steps         : {dict(error_steps) or 'none'}")
    print(f"\nartifact: {out_path}")
