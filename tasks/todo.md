...
- [x] **P0-T02** — Wire `check_doc_sync.py`, `check_sprint_map.py`, `check_no_scope_residue.py` into `.github/workflows/ci.yml`.
  - Progress: added a `Run document governance checks` CI step running all three scripts through `uv run python`; changed the workflow push trigger to run on scratch-branch pushes as well as PRs to `main`, so P0-T03 can be proven without pushing drift to `main`.
...
- [ ] **P0-T03 (exit)** — Prove CI fails on doc/code drift: push a deliberate drift on a scratch branch, watch CI go red, revert. A check that can't fail isn't a check.
  - Progress: **local red-path proof complete** — temporarily inserted `TEMP_DRIFT_PROOF: Terraform` into `docs/SPEC.md`; `uv --cache-dir /tmp/uv-cache run python scripts/ci/check_no_scope_residue.py` failed with `docs/SPEC.md:13: Terraform residue`; removed the marker; reran `check_no_scope_residue.py`, `check_sprint_map.py`, and `check_doc_sync.py` successfully. Remote scratch-branch CI proof remains pending after the Phase 0 commit.
- **Exit:** CI red on governance drift, demonstrated.
