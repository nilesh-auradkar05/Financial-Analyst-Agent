...
- [ ] **P1-T05** — Create evidence release `alpha-evidence:0.1.0` for the current ticker universe; register it.
  - Plan:
    ...
    - [ ] P1-T05c — Run the live capture once for `alpha-evidence:0.1.0`; if provider credentials or network are missing, record the exact blocker and leave the release unapproved.
  - Result: **BLOCKED ON LIVE CAPTURE** — release-builder and live-capture command are implemented and unit-tested. Live attempts after wiring `EDGAR_IDENTITY` into the extractor reached SEC, but SEC rejected both supplied identities (`nilesh-auradkar05`, then `FinancialAnalyst`) as invalid/missing user-agent identities; edgartools requires a value like `Your Name your.email@domain.com`. The runs extracted no usable AAPL sections, no `artifacts/` files were created, and `alpha-evidence:0.1.0` remains unregistered/unapproved.
