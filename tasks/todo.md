...
- [ ] **P1-T05** — Create evidence release `alpha-evidence:0.1.0` for the current ticker universe; register it.
  ...
  - Result: **BLOCKED ON LIVE CAPTURE** — release-builder and live-capture command are implemented and unit-tested. A second live attempt after wiring `EDGAR_IDENTITY` into the extractor reached SEC, but SEC rejected `nilesh-auradkar05` as an invalid/missing user-agent identity; edgartools requires a value like `Your Name your.email@domain.com`. The run extracted no usable AAPL sections, no `artifacts/` files were created, and `alpha-evidence:0.1.0` remains unregistered/unapproved.
