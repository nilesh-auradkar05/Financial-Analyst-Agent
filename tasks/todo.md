...
- [ ] **P1-T05** — Create evidence release `alpha-evidence:0.1.0` for the current ticker universe; register it.
  ...
  - Result: **BLOCKED ON LIVE CAPTURE** — release-builder and live-capture command are implemented and unit-tested, but `uv --cache-dir /tmp/uv-cache run python scripts/dataops/record_evidence_release.py --dataset-version 0.1.0 --max-news 10` stopped before any snapshot was written with `edgar.httprequests.IdentityNotSetException: User-Agent identity is not set` after warning that `EDGAR_IDENTITY` is not set. No `artifacts/` files were created; `alpha-evidence:0.1.0` remains unregistered/unapproved.
