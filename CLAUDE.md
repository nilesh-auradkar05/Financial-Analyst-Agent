# CLAUDE.md — Operating Manual (Alpha Financial Analyst Agent)

This is the operating manual for Claude Code on this repo. It is SPEC-driven, not vibe-coded: every non-trivial change traces to a documented requirement and is verified before it is called done. `AGENTS.md` carries semantically equivalent rules for non-Claude agents. Claude-specific mechanics live here without weakening the shared rules.
...
Use subagents liberally when available to keep the main context clean: one focused task per subagent for research, exploration, and parallel analysis. Subagents do not get to bypass traceability or verification. Apparently even the tiny imaginary interns need supervision.
...
## 10. Claude-specific mechanics

Custom `.claude/` commands and hooks are created manually outside these docs. They must not contradict the hard stops or task loop.

Subagents may be used for research, exploration, and parallel analysis. One focused task per subagent. Main agent remains responsible for integration, traceability, and verification.
